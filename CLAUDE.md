...
- **テスト・lint・format コマンド**: `make test` / `make format` / `make format-check` / `make lint` / `make lint-fix` を使う
...
- 各 commit における整合性チェック（共通 CLAUDE.md の Git 操作セクション参照）の対象に含めるドキュメントは [Spec.md](docs/Spec.md), [Initial-Development.md](docs/archive/Initial-Development.md), [README.md](README.md), [Setup.md](docs/Setup.md)

...
メモリ超過とプロセスの強制終了は予測できないため、CloudWatch のアラームで事後に検知する。関数のエラーメトリクスを監視し、ALARM への遷移を通知トピック経由で Slack に流す。Lambda エントリ点が通常の失敗を捕まえて 500 を返すため、このメトリクスにはランタイム側の終了だけが残る

...
session の途中で作業ディレクトリを移ると、Claude Code はその session を移動先のパスから名付けたプロジェクトに記録するが、最初の `cwd` は移動前のディレクトリのままになる。このため、パスから組み立てた名前が一致し、かついずれかの session が対象ディレクトリを `cwd` として記録しているプロジェクトも該当とみなす。名前の一致だけでは、ドットとパスの区切りの違いしかない別のパスと取り違えるため、`cwd` の記録と組み合わせる
